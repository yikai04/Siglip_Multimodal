"""Create comparison visualizations for Exp20 bad cases.

Run viz_exp20.py first to generate bad case images, then update this script
...
# Update these with actual bad case data after viz_exp20.py runs
bad_cases = [
    # Example: {"caption": "...", "rank": 1234}
    # Fill in after running viz_exp20.py
]

for i, bc in enumerate(bad_cases, 1):
    gt_path = os.path.join(output_dir, f"exp20_gt_badcase_{i}.jpg")
    topk_path = os.path.join(output_dir, f"exp20_badcase_{i}.png")
    ...
    fig.suptitle(f"Exp20 Bad Case #{i}: \"{caption_short}\"", fontsize=14, fontweight="bold", color="#e74c3c", y=1.02)
    fig.legend(handles=legend_elements, loc="lower center", ncol=1, fontsize=12, frameon=True, fancybox=True)
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    save_path = os.path.join(output_dir, f"exp20_badcase{i}_comparison.png")
