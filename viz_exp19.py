"""Generate Exp19 visualizations: heatmap, Top-K retrieval, Bad Cases."""
import argparse
import csv
import os
import random
...
def build_siglip_transform(image_size=224, train=False):
    if train:
        return T.Compose([
...
def plot_heatmap(matrix, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=True, save_path=None, model_source="Exp19"):
    n = matrix.shape[0]
...
def retrieve_topk(query_caption, model, tokenizer, unique_img_embeds, unique_img_ids, device, topk=5):
...
def main():
    ...
    plot_heatmap(logits_matrix, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=True, save_path=os.path.join(args.output_dir, "exp19_logits_heatmap.png"), model_source="Exp19 Pretrained ViT")
    plot_heatmap(cos_sim, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=False, save_path=os.path.join(args.output_dir, "exp19_logits_heatmap_cosine.png"), model_source="Exp19 Pretrained ViT")
    ...
    results = retrieve_topk(query, model, tokenizer, unique_img_embeds, unique_img_ids, device, args.topk)
    for r in results:
        print(f"  Rank {r['rank']}: {r['image_id']}  score={r['score']:.4f}")
    plot_topk_results(query, results, image_root, save_path=os.path.join(args.output_dir, "exp19_topk_query.png"))
    ...
    if bad_cases:
        for i, bc in enumerate(bad_cases[:args.num_bad]):
            ...
            results = retrieve_topk(bc["caption"], model, tokenizer, unique_img_embeds, unique_img_ids, device, args.topk)
            plot_topk_results(bc["caption"], results, image_root, gt_image_id=bc["gt_image_id"],
                             save_path=os.path.join(args.output_dir, f"exp19_badcase_{i+1}.png"),
                             title_prefix=f"[Bad Case #{i+1}  GT-Rank={bc['gt_rank']}]  ")
            # Copy GT image
            import shutil
            gt_src = os.path.join(image_root, bc["gt_image_id"])
            gt_dst = os.path.join(args.output_dir, f"exp19_gt_badcase_{i+1}.jpg")
