...
最优模型为 Exp19（冻结预训练 ViT），配置如下：

\begin{table}[H]
\centering
\caption{最优模型（Exp19）训练配置}
\begin{tabular}{lc}
\toprule
\textbf{参数} & \textbf{值} \\
\midrule
图像编码器 & 冻结的 SigLIP ViT-B/16 (google/siglip-base-patch16-224) \\
文本编码器 & Transformer (2L, 12H, embed\_dim=768), 从头训练 \\
嵌入维度 & 768 \\
训练轮数 & 约 70 (含 resume) \\
Batch size & 64 \\
学习率 & 3e-4 \\
...
\begin{tabular}{lccc}
\toprule
\textbf{指标} & \textbf{Exp19 (预训练ViT)} & \textbf{Exp6 (从头训练)} & \textbf{提升} \\
\midrule
t2i\_R@1 & \textbf{47.36\%} & 36.65\% & +10.71\% \\
t2i\_R@5 & \textbf{72.54\%} & 58.90\% & +13.64\% \\
t2i\_R@10 & \textbf{80.70\%} & 66.76\% & +13.94\% \\
i2t\_R@1 & \textbf{49.97\%} & 39.82\% & +10.15\% \\
i2t\_R@5 & \textbf{74.92\%} & 60.82\% & +14.10\% \\
i2t\_R@10 & \textbf{82.43\%} & 69.30\% & +13.13\% \\
\bottomrule
\end{tabular}
\end{table}

与中期报告基线（常数 LR Transformer，t2i\_R@1=24.81\%）相比，最终最优模型提升了 22.55 个百分点。
...
\begin{enumerate}[nosep]
    \item \textbf{对角线 vs 非对角线}：三个模型的对角线（正样本对）均明显高于非对角线（负样本对），说明模型成功学习到了图文对齐能力。
    \item \textbf{预训练模型优势}：HuggingFace 预训练模型的余弦相似度对角线值最高（约 0.35--0.45），非对角线更低（约 0.05--0.15），正负对比最鲜明。这得益于 ViT 图像编码器和大规模预训练。
...
\begin{enumerate}[nosep]
    ...
    \item 预训练视觉编码器是突破性能天花板的关键——Exp19 冻结 SigLIP ViT-B/16 后 t2i\_R@1 从 36.65\% 跃升至 47.36\%，仅 12 个 epoch 即超过 Exp6 的 100 epoch 结果；
    \item 最终最优模型 t2i\_R@1=47.36\%，相比中期基线提升 22.55 个百分点。
\end{enumerate}

\subsection{改进方向}

\begin{enumerate}
    \item \textbf{端到端微调预训练 ViT}：Exp19 冻结了视觉编码器，若解冻并在小学习率下微调，可能进一步提升视觉-文本对齐精度，但需注意过拟合风险。

    \item \textbf{预训练文本编码器}：当前文本编码器仍从头训练，可引入 BERT 等预训练语言模型初始化，提升文本语义理解能力，特别是对反问句和隐含语义的处理。

    \item \textbf{更大规模数据集}：迁移到 Flickr30k（30000 张图）或 COCO（120000 张图），对比学习需要更多负样本对提供有效训练信号。

    \item \textbf{知识蒸馏}：用完整预训练 SigLIP 模型的输出作为软标签，指导本地模型学习更高质量的嵌入空间，可同时提升视觉和文本编码器。
