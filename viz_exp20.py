"""Generate Exp20 visualizations: heatmap, Top-K retrieval, Bad Cases."""
import argparse
import csv
import os
import random
...
def plot_heatmap(matrix, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=True, save_path=None, model_source="Exp20"):
    n = matrix.shape[0]
...
def main():
    ...
    plot_heatmap(logits_matrix, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=True, save_path=os.path.join(args.output_dir, "exp20_logits_heatmap.png"),
                 model_source="Exp20 Dual Pretrained")
    plot_heatmap(cos_sim, captions, img_ids, logit_scale, logit_bias,
                 use_siglip_scale=False, save_path=os.path.join(args.output_dir, "exp20_logits_heatmap_cosine.png"),
                 model_source="Exp20 Dual Pretrained")
    ...
    plot_topk_results(query, results, image_root, save_path=os.path.join(args.output_dir, "exp20_topk_query.png"))
    ...
    if bad_cases:
        for i, bc in enumerate(bad_cases[:args.num_bad]):
            ...
            plot_topk_results(bc["caption"], results, image_root, gt_image_id=bc["gt_image_id"],
                             save_path=os.path.join(args.output_dir, f"exp20_badcase_{i+1}.png"),
                             title_prefix=f"[Bad Case #{i+1}  GT-Rank={bc['gt_rank']}]  ")
            import shutil
            gt_src = os.path.join(image_root, bc["gt_image_id"])
            gt_dst = os.path.join(args.output_dir, f"exp20_gt_badcase_{i+1}.jpg")
