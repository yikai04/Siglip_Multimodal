"""Train SigLIP with dual pretrained model (Exp20): unfrozen ViT top layers + fine-tuned DistilBERT.

Uses google/siglip-base-patch16-224 ViT-B/16 (bottom frozen, top 3 layers trainable)
and distilbert-base-uncased (fully trainable) with differential learning rates.

Usage:
    python train_siglip_dual_pretrained.py --data-dir Flickr8k --epochs 50 --output-dir outputs/exp20_dual_pretrained
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train dual pretrained SigLIP (Exp20).")
    parser.add_argument("--data-dir", default="Flickr8k")
    parser.add_argument("--output-dir", default="outputs/exp20_dual_pretrained")
    ...
    parser.add_argument("--distilbert-lr", type=float, default=2e-5)
    ...
    parser.add_argument("--distilbert-wd", type=float, default=0.01)
...
def build_dataloaders(args):
    local_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pretrained_distilbert")
...
def main():
    ...
    distilbert_params = list(model.text_model.parameters())
    ...
    db_trainable = sum(p.numel() for p in distilbert_params)
    proj_trainable = sum(p.numel() for p in proj_and_loss_params) - 2  # minus criterion params
    ...
    print(f"LR groups: vit={args.vit_lr}, distilbert={args.distilbert_lr}, proj={args.proj_lr}")
